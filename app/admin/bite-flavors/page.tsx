...
import BiteFlavorManager from '@/components/BiteFlavorManager';

export default function AdminBiteFlavorsPage() {
  ...
  const fetchBiteFlavors = async () => {
    try {
      const response = await fetch('/api/bite-flavors');
      const data = await response.json();
      setBiteFlavors(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error('Failed to fetch bite flavors:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8">
        ...
        <nav className="flex gap-4 flex-wrap">
          ...
          <Link href="/admin/bite-flavors" className="hover:underline font-semibold" style={{ color: '#004AAD' }}>Bite Flavors</Link>
          <Link href="/admin/bite-pricing" className="hover:underline" style={{ color: '#004AAD' }}>Bite Pricing</Link>
      ...
      <BiteFlavorManager biteFlavors={biteFlavors} onRefresh={fetchBiteFlavors} />
    </div>
  );
}
